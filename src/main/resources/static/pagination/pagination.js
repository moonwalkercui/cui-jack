	...
	Page.prototype={
		...
		create:function(){
			...
			if(this.href){
				if(aNum>0){
					...
					if(n>=0){
						for(var i=0;i<this.num;i++){
							...
							a+= "<a href='"+this.href+pagenumber+"' class='ui button "+(i==0?'active':'')+"'>"+pagenumber+"</a>";
						}	
					}else{
						for(var i=n;i<this.count-this.page+1;i++){
							a+= "<a href='"+this.href+(this.page+i)+"' class='ui button "+(this.page+i==this.page?'active':'')+"'>"+(this.page+i)+"</a>";
						}	
					}	
				}else{
					for(var i=0;i<this.count;i++){
						a+= "<a href='"+this.href+(1+i)+"' class='ui button "+(this.page==(1+i)?'active':'')+"'>"+(1+i)+"</a>";
					}	
				}
				var preurl='',nexturl='';
				...
				dom = '<div class="page-contain ui buttons"><a href="'+preurl+'" class=ui button"><</a><div class="page-box ui buttons">'+a+'</div><a href="'+nexturl+'" class="ui button">></a></div>';
			}else{
				if(aNum<0){
					for(var i=0;i<this.count;i++){
						a+= "<a href='javascript:void(0);' class='ui button "+(i==0?'active':'')+"'>"+(1+i)+"</a>";
					}	
				}else{
					for(var i=0;i<this.num;i++){
						a+= "<a href='javascript:void(0);' class='ui button "+(i==0?'active':'')+" "+(this.num-i==1?'page-next':'')+"'>"+(1+i)+"</a>";
					}	
				}
				dom = '<div class="page-contain ui buttons"><a href="javascript:void(0);" class="goPre ui button "><</a><div class="page-box ui buttons">'+a+'</div><a href="javascript:void(0);" class="goNext ui button ">></a></div>';
			}
		...
		next:function(box){
			...
			if(s>0){
				for(var i=0;i<_this.step;i++){
					...
					var dom = "<a href='javascript:void(0);' class='ui button "+(_this.step==(i+1)?'page-next':'')+"'>"+(1+i+n)+"</a>"
				...
				for(var i=0;i<_this.step+s;i++){
					box.removeChild(box.childNodes[0]);
					box.childNodes[0].className+=' page-pre';
					var dom = "<a href='javascript:void(0);' class='ui button "+((_this.step+s)==(i+1)&&((1+i+n)!=_this.count)?'page-next':'')+"'>"+(1+i+n)+"</a>";
					box.appendChild(returnDom(dom));
				}
		...
		pre:function(box){
			...
			if(s>0){
				...
				for(var i=0;i<_this.step;i++){
					...
					var dom = "<a href='javascript:void(0);' class='ui button "+(i==(_this.step-1)&&((n-i-1)!=1)?'page-pre':'')+"'>"+(n-i-1)+"</a>";
				...
				for(var i=0;i<n-1;i++){
					box.removeChild(box.childNodes[_this.num-1]);
					var dom= "<a href='javascript:void(0);' class='ui button'>"+(i+1)+"</a>";
